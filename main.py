...
def main():
    ...
    dilated_img = dilate(markers, color_img)
    find_contours(dilated_img, color_img)
...
# find contours
def find_contours(dilated_img, img_3C):
    print("finding contours")
    contours, hierarch = cv2.findContours(dilated_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cont_counter = 0
    # list_of_bubbles = []
    print("writing contours")
    for cont in contours:
        new_cont = np.zeros(dilated_img.shape, dtype='uint8')
