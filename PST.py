...
def PST(gs_img):
    ...
    L = 0.5
    x = np.linspace(-L, L, gs_img.shape[0])
    y = np.linspace(-L, L, gs_img.shape[1])
    ...
    A = ((np.fft.fft2(gs_img)))
    L = np.fft.fftshift(np.exp(-np.power((np.divide(RHO, math.sqrt((LPF ** 2) / np.log(2)))), 2)))
    # print(L)
    AL = np.real(np.fft.ifft2((np.multiply(A, L))))
    ...
    temp = np.multiply(np.fft.fftshift(np.exp(-1j * PST_Kernel)), np.fft.fft2(AL))
    B = np.fft.ifft2(temp)

    # Calculate phase of the transformed image
    PHI_features = np.angle(B)
    ...
    if Morph_flag == 0:
        ...
        features[gs_img < (np.amax(gs_img) / 20)] = 0  # Removing edges in the very dark areas of the image (noise)
